# Hi, I'm Takiuddin Topu 👋

### Environmental Science | GIS & Remote Sensing | Climate & Environmental Data Analysis | Climate Change | Meterology | Hydrology

I am an Environmental Science graduate from **Islamic University, Bangladesh**, with an interest in understanding environmental and climatic changes through **GIS, remote sensing, and data-driven analysis**.

My academic work has focused on climate variability, extreme precipitation, land use/land cover change, and spatial analysis. I am particularly interested in applying geospatial and statistical methods to environmental problems.

---

## 🌍 Areas of Interest

* Climate and environmental change
* GIS and spatial analysis
* Remote sensing
* Land Use/Land Cover (LULC)
* Climate variability and trends
* Extreme precipitation analysis
* Environmental assessment
* Environmental data analysis

---

## 🛠️ Tools & Technologies

**GIS & Remote Sensing**

* ArcGIS / ArcMap
* QGIS
* Google Earth Engine

**Data Analysis**

* R
* Python
* Microsoft Excel

**Other**

* Statistical analysis
* Spatial analysis
* Environmental assessment
* Scientific research and reporting

---

## 📊 Featured Research

### Undergraduate Thesis

**Spatiotemporal Variability and Trends in Climatic Parameters and Their Association with Extreme Precipitation and Land Use/Land Cover Change in Northwest Bangladesh**

The study investigated long-term variability and trends in rainfall, temperature, relative humidity, and wind speed across **Rajshahi, Pabna, Natore, and Bogura**, along with extreme precipitation characteristics and LULC change.

**Methods included:**

* Mann–Kendall trend analysis
* Sen's slope estimation
* Generalized Extreme Value (GEV) analysis
* Correlation analysis
* GIS-based spatial analysis
* Landsat-based LULC analysis

---

## 📁 Projects

More projects will be added as I continue building my portfolio.

* 🌦️ Climate variability and trend analysis
* 🛰️ Land Use/Land Cover analysis
* 🗺️ GIS and remote sensing projects
* 🌱 Environmental assessment
* 📊 Environmental data analysis

---

## 📫 Connect With Me

* **GitHub:** https://github.com/takiuddintopu
* **LinkedIn:** https://www.linkedin.com/in/takiuddintopu
* **ORCID:** https://orcid.org/0009-0004-3089-7063
